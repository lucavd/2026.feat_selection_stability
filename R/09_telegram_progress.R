...
build_progress <- function(config) {
  sim_dir <- here::here(config$paths$simulated_data)
  fs_dir <- here::here(config$paths$results_fs)

  n_sim <- length(list.files(sim_dir, pattern = "^sim_.*\\.rds$", full.names = FALSE))
  n_methods <- length(config$methods)
  n_total <- n_sim * n_methods
  n_done <- length(list.files(fs_dir, pattern = "^fs_.*\\.rds$", full.names = FALSE))
...
format_message <- function(prog) {
  pct_txt <- if (is.na(prog$pct)) "NA" else sprintf("%.1f", prog$pct)

  paste0(
    "*Feature Selection Progress*\n",
    "Time: `", prog$timestamp, "`\n",
    "Sim datasets: `", prog$n_sim, "`\n",
    "Methods: `", prog$n_methods, "`\n",
    "Completed: `", prog$n_done, " / ", prog$n_total, "` (", pct_txt, "%)\n",
...
main <- function() {
  ...
  if (!watch_mode) {
    prog <- build_progress(config)
    ok <- send_message(bot, chat_id, format_message(prog))
    if (ok) cli::cli_alert_success("Snapshot sent")
    return(invisible(NULL))
  }

  cli::cli_alert_info("Watch mode ON (every {interval_sec}s). Stop with Ctrl+C")
  send_message(bot, chat_id, "*Telegram watcher started*\nMonitoring `R/05_feature_selection.R` progress.")
