...
for (acc_id in successful_ids) {
  ...
  params$mean_control <- colMeans(X[y == 0, , drop = FALSE])
  params$mean_case    <- colMeans(X[y == 1, , drop = FALSE])
  params$sd_control   <- apply(X[y == 0, , drop = FALSE], 2, sd)
  params$sd_case      <- apply(X[y == 1, , drop = FALSE], 2, sd)

  # Overall
  params$mean_overall <- colMeans(X)
  params$sd_overall   <- apply(X, 2, sd)

  # Observed fold changes
  params$observed_fc <- params$mean_case / pmax(params$mean_control, 1e-10)
  params$observed_log2fc <- log2(pmax(params$observed_fc, 1e-10))
  ...
  cli::cli_alert_info("  Median skewness: {round(median(params$skewness), 3)}")
